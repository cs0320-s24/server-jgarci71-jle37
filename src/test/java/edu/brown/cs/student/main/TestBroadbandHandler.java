...
public class TestBroadbandHandler {
  ...
  public void testWeatherRequestSuccess() throws IOException {
    ...
    // Anna: the parameters to this shouldn't matter just because the response will alwayas be
    // constant
    // more so what we want to test is that a proper response to the handler returns a success
    // response, even if
    // the data isn't necessarily correct because it's constant.
    // if you're confused as to why the data is always constant, look at the set up method, and the
    // query method
    // for the MockACS API

    // ONE IMPORTANT THING DONT USE SPACES FOR THE API CALLS USE %20
    ...
    // the response map should ALWAYS return this because our BroadBand Handler is using the mock
    // data
  ...
  //        Map<String, Object> responseBody = adapter.fromJson(new
